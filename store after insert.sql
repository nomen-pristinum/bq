insert into aggregated_deltas_after_fix
select *
from aggregated_deltas_vw
where (address_hex, currency_id) in
      (select hex(transfer_to_bin),
              currency_id
       from example_delete
       union all
       select hex(transfer_from_bin),
              currency_id
       from example_delete
...
insert into current_balances_after_fix
...
insert into cumulative_balances_history_after_fix
select tx_date,
